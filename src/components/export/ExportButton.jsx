...
export function ExportButton({ orders, statistics, dateFilter }) {
  ...
  const exportToCSV = () => {
    ...
    try {
      ...
      link.setAttribute(
        'download',
        `amazon-orders-${dateFilter.label.replace(/\s+/g, '-').toLowerCase()}-${Date.now()}.csv`
      );
  ...
  const exportToJSON = () => {
    ...
    try {
      ...
      link.setAttribute(
        'download',
        `amazon-orders-${dateFilter.label.replace(/\s+/g, '-').toLowerCase()}-${Date.now()}.json`
      );
      link.style.visibility = 'hidden';
      document.body.appendChild(link);
      link.click();
...
--------------------
${statistics.spending.byCategory
  .slice(0, 10)
  .map(
    (cat, i) =>
...
-------------------------
${statistics.products.topBySpending
  .slice(0, 10)
  .map(
    (prod, i) =>
...
═══════════════════════════════════════════════════════════
      ...
      link.setAttribute(
        'download',
        `amazon-summary-${dateFilter.label.replace(/\s+/g, '-').toLowerCase()}-${Date.now()}.txt`
      );
      link.style.visibility = 'hidden';
      document.body.appendChild(link);
      link.click();
