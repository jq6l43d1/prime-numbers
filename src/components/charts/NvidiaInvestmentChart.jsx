...
export function NvidiaInvestmentChart({ orders }) {
  const { loading, error, data } = useOpportunityCostData(orders);

  if (!orders || orders.length === 0) {
    return (
      <Card title="🎮 Nvidia Investment" subtitle="NVDA - AI & Graphics Leader">
  ...
  if (error || !data?.nvidia?.success) {
    return (
      <Card title="🎮 Nvidia Investment" subtitle="NVDA - AI & Graphics Leader">
        <div className="text-center text-gray-500 py-8">
          <p>{error || data?.nvidia?.error || 'Unable to calculate'}</p>
        </div>
      </Card>
    );
  }
  ...
  const { summary, dataPoints } = nvidia;
    ...
    <Card title="🎮 Nvidia Investment" subtitle="NVDA - AI & Graphics Leader">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="bg-red-50 rounded-lg p-3">
          ...
          <p className="text-lg font-bold text-red-700">${formatNumber(summary.currentValue, 2)}</p>
        </div>
      ...
      {summary.excludedOrders > 0 && (
        <div className="mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded text-xs text-yellow-800">
          ℹ️ {summary.excludedOrders} orders before Jan 1999 excluded (NVDA IPO)
        </div>
      )}
    </Card>
  );
