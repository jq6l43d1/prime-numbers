...
export function AmazonInvestmentChart({ orders }) {
  const { loading, error, data } = useOpportunityCostData(orders);

  if (!orders || orders.length === 0) {
    return (
      <Card title="🛍️ Amazon Stock Investment" subtitle="AMZN - The Company">
  ...
  if (error || !data?.amazon?.success) {
    return (
      <Card title="🛍️ Amazon Stock Investment" subtitle="AMZN - The Company">
        <div className="text-center text-gray-500 py-8">
          <p>{error || data?.amazon?.error || 'Unable to calculate'}</p>
        </div>
      </Card>
    );
  }
  ...
  const { summary, dataPoints } = amazon;
    ...
    <Card title="🛍️ Amazon Stock Investment" subtitle="AMZN - The Company">
      ...
      {summary.excludedOrders > 0 && (
        <div className="mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded text-xs text-yellow-800">
          ℹ️ {summary.excludedOrders} orders before May 1997 excluded (AMZN IPO)
        </div>
      )}
    </Card>
  );
