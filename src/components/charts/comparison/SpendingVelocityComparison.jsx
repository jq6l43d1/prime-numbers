...
export function SpendingVelocityComparison({ datasetStats }) {
  ...
  velocityData.forEach(({ monthlyData }) => {
  ...
  const datasets = datasetStats.map((item, index) => {
    const monthlyData = velocityData[index].monthlyData;
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Spending Velocity Comparison (Cumulative)',
        font: {
          size: 16,
          weight: 'bold'
        }
